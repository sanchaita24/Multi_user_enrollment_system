                ...
                <table class="table table-bordered text-center align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <!-- Edit -->
                                <a href="{{ ('/user_edit/'. $info->id)  }}">
                                    <button class="btn btn-primary btn-sm">Edit</button></a>


                                <!-- Delete -->
                                <a href="{{  ('/user_delete/'. $info->id) }}"
                                    onclick="return confirm('Are you sure to delete?')">
                                    <button class="btn btn-danger btn-sm">Delete</button></a>


                                <!-- Purchase Course -->
                                <a href="{{ url('/check_enrollment/'.$info->id )}}">
                                    <button class="btn btn-success btn-sm">Enroll Course</button></a>


                                <!-- block/unblock code -->
                                @if($info->role==0)
                                 <a href="{{ url('/block/'.$info->id )}}">
                                    <button class="btn btn-danger btn-sm">Block</button></a>

                                   @else
                                   <a href="{{ url('/unblock/'.$info->id )}}">
                                    <button class="btn btn-success btn-sm">Unblock</button></a>
                               @endif
                            </td>
                        </tr>
                        @endforeach
